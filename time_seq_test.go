...
func TestExportLine(t *testing.T) {
	...
	got := line.Export()
	if got != "2020-04-01,16:00,17:30,MYPROJECT-2021,second activity" {
		t.Error("Export format is wrong", got)
	}
}

func TestFindTicketNumber(t *testing.T) {
	commit := "MYPROJECT-2020 some work done"
	got, err := GetTicketNumber(commit)
	if err != nil {
		t.Error(err)
	}
	if got != "MYPROJECT-2020" {
...
func TestSplitHGLog(t *testing.T) {
	log := `1589393622|MYPROJECT-2548 deploy 3.20.1 on QA
1589378723|MYPROJECT-2548 deploy 3.20.1 on staging`
	got := SplitLog(log)
	if len(got) != 2 {
...
func TestParseLineFromLogEntry(t *testing.T) {
	logline := "1588090089.0-10800|MYPROJECT-2546 absolute import"
	location, _ := time.LoadLocation("Europe/Moscow")
	expected := Line{
